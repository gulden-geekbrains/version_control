...
```
14. Затем, на Github-е появится ветка и её содержимое, которое мы только что отправили. 
15. Жмём кнопку `Contribute`, а в её выпадайке `Open pull request`.
16. Переходим в меню pull request-а, там заполняем комментарий и описание нашей правки или дополнения.
17. Жмём кнопку `Create pull request`
